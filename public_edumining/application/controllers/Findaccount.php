...
class Findaccount extends CB_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model('Member_model','MM',TRUE);
    }
        ...
        if ($this->form_validation->run() === false) {
            ...
            if ($this->input->post('findtype') === 'findidpw') {
                ...
                */
                ...
                // 비밀번호 초기화
                $initPassword = element('mem_userid', $mb);
                $hash = password_hash($initPassword, PASSWORD_BCRYPT);
                
                $updatedata = array(
                		'mem_password' => $hash,
                );
				if ($this->input->post('idpw_name') == element('mem_username', $mb)){
					$this->Member_model->update($mem_id, $updatedata);
					
					$view['view']['message'] = '회원님의 비밀번호를 초기화 하였습니다. <br/><br/> &emsp; - 아이디 : '.element('mem_userid', $mb).'<br /> &emsp; - 비밀번호 : '.$initPassword;
				}else{
					$view['view']['message'] = '회원님의 정보가 일치하지 않습니다. <br/><br/> &emsp;확인 후 다시 입력하세요.';
				}
    ...
    public function _existemail($str)
    {
        $userinfo = $this->Member_model
            ->get_by_email($str, 'mem_id, mem_email, mem_email_cert');
        ...
        if ($this->cbconfig->item('use_register_email_auth') && ! element('mem_email_cert', $userinfo)) {
            $this->form_validation->set_message(
                '_existemail',
    ...
    public function _check_id_pw($password, $userid)
    {
        ...
        $userselect = 'mem_id, mem_password';
    ...
    public function check_name_email() {
        ...
        $result = $this->MM->getMemberByNameEmail($queryCondition);

        $userId = count($result) > 0 ? $result[0]["mem_userid"] : '';
        if($userId == '') {
        ...
        $initPassword = $userId;
        $hash = password_hash($initPassword, PASSWORD_BCRYPT);
        $updatedata = array("user_id" => $userId, "password" => $hash);
        $this->MM->updatePassword($updatedata);
        echo json_encode(array(
            "is_success"   => true,
            "user_id"      => $userId,
            "new_password" => $initPassword
        ));
    }
}
