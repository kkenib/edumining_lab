...
Calendar.prototype.create = function (_par) {
	...
	hh("X", 1, 100).ttip = Calendar._TT["CLOSE"];
	this.title = hh("", this.isPopup ? 5 : 6, 300);
	this.title.className = "title";
	if (this.isPopup) {
		this.title.ttip = Calendar._TT["DRAG_TO_MOVE"];
		this.title.style.cursor = "move";
		hh("X", 1, 200).ttip = Calendar._TT["REMOVE"];
	}
...
Calendar.prototype._init = function (mondayFirst, date) {
				...
				if ((!i && j < wday) || iday > no_days) {
					...
					if (this.disabledHandler && this.disabledHandler(date)) {
						...
						cell.ttip =
							...
							"월 " + iday + "일 (" + Calendar._DN[wday + MON].substr(0, 3) + ")";
						Calendar._add_evs(cell);
						if (iday == mday) {
	...
	this.title.firstChild.data = year + "년 " + Calendar._MN[month] + "월";
};
...
Calendar._TT["PREV_YEAR"] = "이전 년도 (드랍메뉴)";
Calendar._TT["PREV_MONTH"] = "이전 달 (드랍메뉴)";
Calendar._TT["GO_TODAY"] = "오늘 날짜로 가기";
Calendar._TT["NEXT_MONTH"] = "다음 달 (드랍메뉴)";
Calendar._TT["NEXT_YEAR"] = "다음 년도 (드랍메뉴)";
Calendar._TT["SEL_DATE"] = "날짜를 선택하세요";
Calendar._TT["DRAG_TO_MOVE"] = "메뉴이동";
Calendar._TT["PART_TODAY"] = " (오늘)";
Calendar._TT["MON_FIRST"] = "Display Monday first";
Calendar._TT["SUN_FIRST"] = "Display Sunday first";
Calendar._TT["CLOSE"] = "닫기";
Calendar._TT["TODAY"] = "오늘";
Calendar._TT["REMOVE"] = "지우기";
...
function showCalendar(id, format) {
  ...
  el.focus();
  $("#channelNaver, #channelDaum, #channelGoogle, #channelFacebook, #channelTwitter, #channelYoutube").hide();
  return false;
}
