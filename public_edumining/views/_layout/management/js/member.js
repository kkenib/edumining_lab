...
function select_identity(frm, identity){
	switch (identity)
	{
	case 'professor' : 
		identity = "교수";
	...
	frm.identity.value = identity;
	if (identity == 'etc')
	{
		$("#identity").prop("readonly", false);
		$("#identity").val("");
		$("#identity").attr("placeholder", "직접 입력하세요");
	}else
	{
...
function check_member_submit_modify(frm) {
	if(frm == null) frm = document.frm;
	var identityTemp = $("#identity").val();
	var birthData = $("#birth_temp").val();
	birthData = birthData.split('-');

	var birthYear = $("#year option:selected").val();
	var birthMonth = $("#month option:selected").val();
	var birthDay = $("#day option:selected").val();

	var existingYear = birthData[0];
	var existingMonth = birthData[1];
	var existingDay = birthData[2];

	if (birthYear == "")
	{
		birthYear = existingYear;
	}
	if (birthMonth == "")
	{
		birthMonth = existingMonth;
	}
	if (birthDay == "")
	{
		birthDay = existingDay;
	}

	if (isNaN(birthYear) == true || isNaN(birthMonth) == true || isNaN(birthDay) == true)
	{
		alert("생년월일을 입력해 주세요");
	}
	...
	if (birthMonth.length == 1 && birthDay.length == 1)
	{
		...
		var birthMonth = "0" + birthMonth;
	}else{
		birthMonth = birthMonth;
		birthDay = birthDay;
	}
	
	var birthCheck = birthYear + birthMonth + birthDay;
	$("#birth").val(birthCheck);
	...
	//if($.trim(frm.zip.value)=="" || $.trim(frm.add1.value)=="") {
	//if($.trim(frm.zip1.value)=="" && $.trim(frm.zip2.value)=="" && $.trim(frm.add1.value)=="" && $.trim(frm.zip_new.value)=="") {
	//	alert("주소를 입력하세요.");
	//	frm.add1.focus();
	//	return false;
	//}
